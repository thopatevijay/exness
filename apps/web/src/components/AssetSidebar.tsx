...
export function AssetSidebar({ selected, onSelect }: Props) {
  ...
  return (
    <aside className="border-r border-[color:var(--color-border)]">
      ...
      <ul>
        {assets.map((a) => {
          const stale = a.ts !== null && Date.now() - a.ts > STALE_AFTER_MS;
          return (
            <li key={a.symbol}>
              <button
              ...
              >
                ...
                <span className="text-right">
                  ...
                  <PriceCell value={a.buyPrice} decimals={a.decimals} stale={stale} />
                  <div className="text-xs text-[color:var(--color-fg-dim)]">Sell</div>
                  <PriceCell value={a.sellPrice} decimals={a.decimals} stale={stale} />
                </span>
              </button>
            </li>
          );
